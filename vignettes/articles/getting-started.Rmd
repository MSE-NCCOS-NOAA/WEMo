...
```{r read-bathy-data-for-show, eval = FALSE}
# load bathymetry
...
```{r bathy-path-hidden, echo = F, message=FALSE, fig.alt = "Map showing Bathymetric raster"}
...
```{r import-and-plot-points, fig.alt="Map showing site points"}
# Plot site points
...
```{r import-and-plot-shoreline, fig.alt="Map showing MHHW shoreline"}
# Plot shoreline
...
```{r plot-shoreline-and-points, fig.alt="Map showing bathymetrey, shoreline, and points"}
# plot all input spatial data
ggplot()+
  geom_spatraster(data = PI_bathy)+
  geom_sf(data = PI_shoreline, fill = "honeydew3")+
...
```{r plot-wind-rose, eval=TRUE, fig.alt="Wind Rose"}
...
```{r display-final-wemo-results, fig.alt="Map Showing the results of WEMo model"}
...
```{r display-wemo-details, fig.alt="Map waves modelled by WEMo for each fetch ray"}
...
```{r make-variables-to-output, include=FALSE}
# Calculate the differences for the first site (or use max() across all sites)
diff_max <- max(results_windy$wemo_final$max_wave_height - results$wemo_final$max_wave_height)
diff_avg <- max(results_windy$wemo_final$avg_wave_height - results$wemo_final$avg_wave_height)

# Convert to cm and round
diff_max_cm <- round(diff_max * 100, 1) 
diff_avg_cm <- round(diff_avg * 100, 1)

```

We see increases in wave heights up to `r diff_max_cm` cm for max wave heights and `r diff_avg_cm` cm for average wave heights. We also see a corresponding increase in RWE.

## Comparing Results Scenarios

This example demonstrates advanced visualization techniques for comparing scenarios.

```{r Comparing-Results-Scenarios, fig.dim=c(9, 6), fig.alt="Map comparing WEMo results under different scenarios"}
