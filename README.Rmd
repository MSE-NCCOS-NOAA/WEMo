...
* **Data Preparation Tools**: Includes convenience functions to download and prepare input data, such as historical wind observations from NOAA's [Global Historic Climate Network hourly] dataset (`get_wind_data()`) and bathymetry from NOAA's [Continuously Updated Digital Elevation Model]{https://www.ncei.noaa.gov/products/coastal-elevation-models} dataset (`get_noaa_cudem()`).
