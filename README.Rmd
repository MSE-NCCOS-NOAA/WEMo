...
* **Data Preparation Tools**: Includes convenience functions to download and prepare input data, such as historical wind observations from NOAA's [Global Historic Climate Network hourly](https://www.ncei.noaa.gov/products/global-historical-climatology-network-hourly) dataset (`get_wind_data()`) and bathymetry from NOAA's [Continuously Updated Digital Elevation Model](https://www.ncei.noaa.gov/products/coastal-elevation-models) dataset (`get_noaa_cudem()`).
...
``` {r display how to install package, eval=FALSE}
...
* `PI_shoreline`: a polygon of the Mean Higher High Water shoreline (0.522 m NAVD88). A `sf` MULTIPOLYGON object.
* `PI_wind_data`: hourly wind history from a nearby weather station. A `tibble`.
* `PI_bathy`: a topobathy raster storing elevations in m NAVD88. A `SpatRaster`,

```{r examine_input_data, fig.alt="Map showing input spatial data"}
...
```{r summarize_and_plot_wind_data, fig.alt="Wind rose plot"}
...
```{r plot_final_results, fig.alt="Map of WEMo results"}
...
* **Wind Data**: Download historical wind data from NOAA's Global Historical Climatology Network hourly (GHCNh) using `get_wind_data()`.
...
For a detailed, step-by-step tutorial on using these functions, please see the [*"Gathering Input Data for WEMo"* article](https://github.com/MSE-NCCOS-NOAA/WEMo/blob/HEAD/vignettes/articles/gathering-inputs.Rmd).
...
> Walker, Q.A. (2026). *WEMo: An R Implementation of the Wave Exposure Model*. R package version `r packageVersion("WEMo")` https://github.com/MSE-NCCOS-NOAA/WEMo
